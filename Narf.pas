...
{
----------------------------------------------------------------
TNarfStreams v1.7 by Harry Kakoulidis 01/2002
prog@xarka.com
...
procedure Register;
begin
  RegisterComponents('NARF',[TNarfStreams]);
end;

{ TNarfStreams }

procedure TNarfStreams.AddStream(const ID: string; Source: TStream);
...
procedure TNarfStreams.ClearStreams;
var a:integer;
begin
...
constructor TNarfStreams.Create(AOWner: TComponent);
begin
...
destructor TNarfStreams.Destroy;
begin
 ClearStreams;
 StreamList.free;
 inherited Destroy;
...
procedure TNarfStreams.GetStream(const ID: string; Dest: TStream);
var i:integer;
begin
...
procedure TNarfStreams.LoadFromFile(const Filename: string);
var fs:TFileStream;
begin
...
Procedure TNarfStreams.FoundCorrupt;
begin
...
function TNarfStreams.CheckGood(ms : TStream) : boolean;
var
...
procedure TNarfStreams.LoadFromStream(ms: TStream);
...
procedure TNarfStreams.LoadFromStreamNor(ms: TStream);
var
 Mem : TMemoryStream;
...
function TNarfStreams.ReadStr(Stream: TStream): string;
var
 i:word;
 s:string;
begin
...
procedure TNarfStreams.RemoveStream(const ID: String);
var i:integer;
begin
 if (length(id)>0) then begin
  i:=Streamlist.IndexOf(ID);
...
procedure TNarfStreams.SaveToFile(const Filename: string);
var fs:TFileStream;
begin
...
procedure TNarfStreams.SaveToStream(ms: TStream);
var
...
procedure TNarfStreams.SaveToStreamNor(ms : TStream);
var
 Count,size,a : integer;
begin
...
procedure TNarfStreams.WriteStr(S: String; Stream: TStream);
var
 i:word;
begin
 i:=length(s);
